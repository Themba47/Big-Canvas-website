...
function startCanvasAnim(bc, tc, gc, img) {
  ...
  var draw = function (bufferLength, wow, barWidth, barHeight, dataArray) {
    ...
    canvasCtx.beginPath();
    for (let k = 0, v = 0; k < canvas.height; k += barWidth, v++) {
      if (v == 2) {
        v = 0;
      }
      if (v == 1) {
      ...
      canvasCtx.fillRect(0, k, canvas.width, barWidth);
    }

    canvasCtx.beginPath();

    canvasCtx.lineWidth = barWidth / 2;
    canvasCtx.strokeStyle = soul.text_color[1];
    canvasCtx.moveTo(wow - barWidth, canvas.height - barHeight);

    for (let i = 0, k = 0; i < bufferLength; i++, k++) {
      if (k == 2) {
        k = 0;
      }
      ...
      // canvasCtx.lineTo(wow, canvas.height - barHeight);
