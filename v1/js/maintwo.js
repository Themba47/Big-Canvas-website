...
<textarea maxlength="3000" name="leavemsg" id="leavemsg" cols="30" rows="10" placeholder="leave your input on how we can improve the game"></textarea><br><br>
<input type="text" name="leavemsg_name" id="leavemsg_name" placeholder="Your name"><br>
...
window.addEventListener("load", () => {
  ...
  if (mylvl) {
    console.log(localStorage.getItem("mylevel"));
    console.log(localStorage.getItem("mylives") + " Lives");
    getQuestions(localStorage.getItem("mylevel"));
    userLevel.innerHTML = `<label style="color: #fff;">Level ${localStorage.getItem(
      "mylevel"
    )}</label>`;
  } else {
    getQuestions(1);
      ...
      .then((data) => {
        ...
        if (data == 200) {
          document.getElementById("leavemsg_div").innerHTML =
            "<h4>Thank you for your input</h4>";
        }
      });
  }
}
